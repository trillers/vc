var channels = require('../channels-setting');
var valueToKey = require('../util').valueToKey;

function Service(mqPubClient, mqSubClient){
    this.pubClient = mqPubClient;
    this.subClient = mqSubClient;
    this._handlersMap = {};
}

var proto = Service.prototype;

proto.publish = function(channel, message){
    this.pubClient.publish(channel, message);
};

proto.subscribe = function(channel){
    this.subClient.subscribe(channel)
};

proto.subscribeAll = function(){
    for(var prop in channels){
        this.subClient.subscribe(channels[prop]);
    }
};

proto.init = function(){
    var self = this;
    this.subClient.on('message', self._handleMessage.bind(self));
};

proto._handleMessage = function(channel, message){
    var method = valueToKey(channels, channel);
    if(method){
        this._handlersMap[method].forEach(function(fn){
            fn.apply(null, [JSON.parse(message)]);
        })
    }else{
        console.info('no such method in channels');
    }
};

proto.startUp = function(){
    this.pubClient.publish(channels.startUp);
};

proto.onStartUp = function(bid, handler){
    this.subClient.subscribe(channels.startUp);
    if(!this._handlersMap['startUp']){
        this._handlersMap['startUp'] = [];
    }
    this._handlersMap['startUp'].push(handler);
};

proto.heartBeatResponse = function(data){
    this.pubClient.publish(channels.heartBeatResponse, JSON.stringify(data));
};

proto.onHeartBeatResponse = function(bid, handler){
    this.subClient.subscribe(channels.heartBeatResponse);
    if(!this._handlersMap['heartBeatResponse']){
        this._handlersMap['heartBeatResponse'] = [];
    }
    this._handlersMap['heartBeatResponse'].push(handler);
};

proto.heartBeatRequest = function(bid){
    this.pubClient.publish(channels.heartBeatRequest, JSON.stringify({bid: bid}));
};

proto.onHeartBeatRequest = function(bid, handler){
    this.subClient.subscribe(channels.heartBeatRequest);
    if(!this._handlersMap['heartBeatRequest']){
        this._handlersMap['heartBeatRequest'] = [];
    }
    this._handlersMap['heartBeatRequest'].push(handler);
};

proto.agentHeartBeatRequest = function(bid){
    this.pubClient.publish(channels.agentHeartBeatRequest, JSON.stringify({bid: bid}));
};

proto.onAgentHeartBeatRequest = function(bid, handler){
    this.subClient.subscribe(channels.agentHeartBeatRequest);
    if(!this._handlersMap['agentHeartBeatRequest']){
        this._handlersMap['agentHeartBeatRequest'] = [];
    }
    this._handlersMap['agentHeartBeatRequest'].push(handler);
};

proto.agentHeartBeatResponse = function(bid){
    this.pubClient.publish(channels.agentHeartBeatResponse, JSON.stringify({bid: bid}));
};

proto.onAgentHeartBeatResponse = function(bid, handler){
    this.subClient.subscribe(channels.agentHeartBeatResponse);
    if(!this._handlersMap['agentHeartBeatResponse']){
        this._handlersMap['agentHeartBeatResponse'] = [];
    }
    this._handlersMap['agentHeartBeatResponse'].push(handler);
};

module.exports = function(pub, sub){
    return new Service(pub, sub);
};