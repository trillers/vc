    ...
    ch.assertQueue(queueName, conf.queueOptions)
        .then(function(){
            ...
            ch.consume(queueName, function(msg) {
                self.emit(eventName, null, JSON.parse(msg.content.toString()), msg);
                if(!conf.consumeOptions.noAck && !conf.ackDelay){
                    ch.ack(msg);
                }
            }, conf.consumeOptions);
        });
...
/**
 * acknowledge the queue msg has handled
 * @param ch
 * @param msg
 */
helper.acknowledge = function(ch, msg){
    ch.ack(msg);
}
